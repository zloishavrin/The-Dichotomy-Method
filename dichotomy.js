...
function createTableHead(table) {
	...
	let tableHeaders = ['Iteration Number', 'First Root', 'Second Root' ,
	'Intermediate Root', 'Function from First Root', 'Function from Second Root',
	'Function from Intermediate Root', 'Root of Root Difference'];
...
function createTableElement(tableBody, value1, value2, value3, 
	...
	let arrayRow = [value1, value2, value3, value4, value5, value6, value7, value8];
...
function dichotomyMethod() {
	...
	let iteration = {
		...
		intermediateRoot: 1, //intermediate root
	...
	let degreePrecision = 0; //for further rounding
	...
	while(Math.abs(iteration.secondRoot-iteration.firstRoot) >= precision) {
		...
		iteration.intermediateRoot = (iteration.firstRoot + iteration.secondRoot) / 2;

		//creating a table row that corresponds to a loop iteration
		createTableElement(tableBody, iteration.num, iteration.firstRoot, 
			iteration.secondRoot, iteration.intermediateRoot, 
			f(iteration.firstRoot), f(iteration.secondRoot), 
			f(iteration.intermediateRoot), 
			Math.abs(iteration.secondRoot-iteration.firstRoot));

		//Check which way to move the root
		if(f(iteration.secondRoot)*f(iteration.intermediateRoot) < 0) {

			iteration.firstRoot = iteration.intermediateRoot;
		...
		//magnification and root calculation are here for the last iteration
		iteration.num++;
		iteration.intermediateRoot = (iteration.firstRoot + iteration.secondRoot) / 2;
	...
	tdText = document.createTextNode(iteration.intermediateRoot.toFixed(degreePrecision+1));
